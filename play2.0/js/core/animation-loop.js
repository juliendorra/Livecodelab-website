...
define(function() {
  ...
  AnimationLoop = (function() {
    ...
    AnimationLoop.prototype.animate = function() {
      var drawFunctionRunner, e, forbiddenZone, frameStartTime, geometryOnScreenMightHaveChanged;
      if (this.frame === 0) {
        this.liveCodeLabCoreInstance.timeKeeper.resetTime();
      } else {
        this.liveCodeLabCoreInstance.timeKeeper.updateTime();
      }
      frameStartTime = this.liveCodeLabCoreInstance.timeKeeper.milliseconds;
      forbiddenZone = Math.min(Math.max.apply(Math, this.fpsHistory), 1000 / 30);
      if (this.liveCodeLabCoreInstance.timeKeeper.nextQuarterBeat - frameStartTime < forbiddenZone) {
      ...
      if (this.liveCodeLabCoreInstance.drawFunctionRunner.drawFunction) {
        ...
        drawFunctionRunner.putTicksNextToDoOnceBlocksThatHaveBeenRun(this.liveCodeLabCoreInstance.codeCompiler);
      } else {
      ...
      geometryOnScreenMightHaveChanged = this.liveCodeLabCoreInstance.graphicsCommands.atLeastOneObjectWasDrawn || this.liveCodeLabCoreInstance.graphicsCommands.atLeastOneObjectIsDrawn;
      if ((!this.noDrawFrame) && geometryOnScreenMightHaveChanged) {
        this.liveCodeLabCoreInstance.renderer.render(this.liveCodeLabCoreInstance.graphicsCommands);
        this.liveCodeLabCoreInstance.graphicsCommands.atLeastOneObjectWasDrawn = this.liveCodeLabCoreInstance.graphicsCommands.atLeastOneObjectIsDrawn;
        this.fpsHistory.push(new Date().getTime() - frameStartTime);
    ...
    AnimationLoop.prototype.cleanStateBeforeRunningDrawAndRendering = function() {
      this.liveCodeLabCoreInstance.renderer.resetExclusionPrincipleWobbleDataIfNeeded(this.liveCodeLabCoreInstance.graphicsCommands);
      ...
      return this.liveCodeLabCoreInstance.backgroundPainter.resetGradientStack();
    };
